...
bool EntityEditorApp::Startup() {
	fileHandleData = CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(Entity), L"fileHandleData");
	fileHandleSize = CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(int), L"fileHandleSize");
	...
	// Writing
	if (fileHandleData == nullptr) { std::cout << "Could not create file mapping object: " << GetLastError() << std::endl; return 1; }

	/*unsigned char* data = (unsigned char*)MapViewOfFile(fileHandleData, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(Entity) * ENTITY_COUNT);
...
void EntityEditorApp::Update(float deltaTime) {
	...
	data = (Entity*)MapViewOfFile(fileHandleData, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(Entity) * ENTITY_COUNT);
